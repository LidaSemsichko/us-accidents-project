...
def to_bronze(raw_filename):
    ...
    df.write.mode("overwrite").format("delta").option("overwriteSchema", "true").partitionBy("event_date").saveAsTable(bronze_table)
    df.write.mode("overwrite").partitionBy("event_date").parquet(bronze_path)
...
def to_silver():
    ...
    core_cols = [c for c in ["ID", "Severity", "Start_Time", "End_Time", "Start_Lat", "Start_Lng"] if c in df.columns]
    if core_cols:
